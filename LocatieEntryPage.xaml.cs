...
public partial class LocatieEntryPage : ContentPage
{
    ...
    {
        base.OnAppearing();
        listView.ItemsSource = await App.Database.GetLocatiiAsync();
    }
    ...
    {
        if (e.SelectedItem != null) { await Navigation.PushAsync(new LocatiePage { BindingContext = e.SelectedItem as Locatie }); }
    }
}